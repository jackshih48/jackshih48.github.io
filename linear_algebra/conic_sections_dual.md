...
若取平面上一點 $P(\mathbf{x}_0)$，則可對應到一直線：
...
當 $P$ 在二次曲線上時，此線為切線；當 $P$ 不在二次曲線上時，此線稱為 $P$ 對此二次曲線的**極線**。  
反過來，點 $P$ 稱為此極線的**極點**。
...
這個性質稱為**極點與極線的對稱性**，也稱為 **La Hire 定理**。

<br>

## 由極線看二次曲線的對偶結構

由上面的對稱性可知，二次曲線不只給出切線公式，也在平面上建立了點與線之間的對應關係。

對於平面上每一點 $P$，上述公式都給出一條對應直線 $\ell_P$：
...
由 La Hire 定理，還可得到下列對應。

若兩點 $A,B$ 都在某點 $P$ 的極線上，則由對稱性可知，點 $P$ 也必在 $A,B$ 的極線上。  
也就是說：

- 若一個點在另外兩點的極線上，
- 則那兩點的極線交於此點。

換句話說，原本「兩點決定一直線」的結構，經過極線對應後，會轉成「兩線交於一點」的結構。

這也呼應了前面「當點不在曲線上」一節中的幾何現象：  
切點的連線、切線的交點，都可視為對偶結構的表現。

在二次曲線所誘導的幾何中，常見的對應關係如下：
...
因此，為了補足這些缺口，並保留點線對偶的完整結構，我們需要引入**齊次座標**，將仿射平面擴充為**射影平面**。
...
- 再由切線公式推廣到極線；
- 進一步看見點與線之間的對偶結構；
- 最後發現：若要使此結構沒有例外，就必須進入射影平面。

這便是由仿射二次曲線走向射影對偶的自然過程。